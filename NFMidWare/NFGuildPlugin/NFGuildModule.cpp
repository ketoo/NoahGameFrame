...
bool NFGuildModule::Shut()
{
    return true;
}

bool NFGuildModule::Execute()
{
    return true;
}

bool NFGuildModule::AfterInit()
{
	if (!m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_REQ_CREATE_GUILD, this, &NFGuildModule::OnCreateGuildProcess)) { return false; }
	if (!m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_REQ_JOIN_GUILD, this, &NFGuildModule::OnJoinGuildProcess)) { return false; }
	if (!m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_REQ_LEAVE_GUILD, this, &NFGuildModule::OnLeaveGuildProcess)) { return false; }
	if (!m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_REQ_OPR_GUILD, this, &NFGuildModule::OnOprGuildMemberProcess)) { return false; }
	if (!m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_REQ_SEARCH_GUILD, this, &NFGuildModule::OnSearchGuildProcess)) { return false; }
	if (!m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_REQ_CHAT, this, &NFGuildModule::OnClientChatProcess)) { return false; }

	m_pKernelModule->AddClassCallBack(NFrame::Guild::ThisName(), this, &NFGuildModule::OnObjectClassEvent);

	m_pKernelModule->RegisterClassPropertyEvent(NFrame::Guild::ThisName(), this, &NFGuildModule::OnPropertyCommonEvent);
	m_pKernelModule->RegisterClassRecordEvent(NFrame::Guild::ThisName(), this, &NFGuildModule::OnRecordCommonEvent);

    return true;
}

bool NFGuildModule::CreateGuild(const NFGUID& self, const NFGUID& xGuildID, const std::string& strGuildName)
{
...
bool NFGuildModule::JoinGuild(const NFGUID& self, const NFGUID& xGuildID)
{
...
bool NFGuildModule::LeaveGuild(const NFGUID& self, const NFGUID& xGuildID)
{
	NF_SHARE_PTR<NFIRecord> xMemberRecord = m_pKernelModule->FindRecord(xGuildID, NFrame::Guild::Guild_MemberList::ThisName());
	if (xMemberRecord)
	{
...
bool NFGuildModule::PromotionMember(const NFGUID& self, const NFGUID& xGuildID, const NFGUID& xMember)
{
...
bool NFGuildModule::DemotionMember(const NFGUID& self, const NFGUID& xGuildID, const NFGUID& xMember)
{
	NFIGuildModule::MEMBER_TYPE eMemberLevelOperater = CheckPower(self, xGuildID);
	NFIGuildModule::MEMBER_TYPE eMemberLevel = CheckPower(xMember, xGuildID);
	if (eMemberLevelOperater == MEMBER_TYPE::MT_NONE
...
bool NFGuildModule::MemberOnline(const NFGUID& self, const NFGUID& xGuild)
{
...
bool NFGuildModule::MemberOffline(const NFGUID& self, const NFGUID& xGuild)
{
	NF_SHARE_PTR<NFIRecord> xRecord = m_pKernelModule->FindRecord(xGuild, NFrame::Guild::ThisName());
	const int nRow = xRecord->FindObject(NFrame::Guild::Guild_MemberList::GUID, self);
	if (nRow >= 0)
...
bool NFGuildModule::AddMember(const NFGUID & xGuildID, const NFGUID & player, const MEMBER_TYPE type)
{
	NF_SHARE_PTR<NFIRecord> xMemberRecord = m_pKernelModule->FindRecord(xGuildID, NFrame::Guild::Guild_MemberList::ThisName());
	if (xMemberRecord)
	{
...
NFIGuildModule::MEMBER_TYPE NFGuildModule::CheckPower(const NFGUID& self, const NFGUID& xGuildID)
{
	NF_SHARE_PTR<NFIRecord> xMemberRecord = m_pKernelModule->FindRecord(xGuildID, NFrame::Guild::Guild_MemberList::ThisName());
	if (xMemberRecord)
	{
...
void NFGuildModule::OnGuildOnlineProcess(const NFGUID& xGuildID)
{

}

void NFGuildModule::OnGuildOfflineProcess(const NFGUID& xGuildID)
{

}

void NFGuildModule::OnCreateGuildProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
...
void NFGuildModule::OnJoinGuildProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	CLIENT_MSG_PROCESS_NO_OBJECT(nMsgID, msg, nLen, NFMsg::ReqAckJoinGuild);
}

void NFGuildModule::OnLeaveGuildProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	CLIENT_MSG_PROCESS_NO_OBJECT(nMsgID, msg, nLen, NFMsg::ReqAckLeaveGuild);
}

void NFGuildModule::OnOprGuildMemberProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	CLIENT_MSG_PROCESS_NO_OBJECT(nMsgID, msg, nLen, NFMsg::ReqAckOprGuildMember);
}

void NFGuildModule::OnSearchGuildProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	CLIENT_MSG_PROCESS_NO_OBJECT(nMsgID, msg, nLen,NFMsg::ReqSearchGuild);

}

void NFGuildModule::OnClientChatProcess(const NFSOCK nSockIndex, const int nMsgID, const char *msg, const uint32_t nLen)
{
...
int NFGuildModule::OnObjectClassEvent(const NFGUID & self, const std::string & strClassName, const CLASS_OBJECT_EVENT eClassEvent, const NFDataList & var)
{
	return 0;
}

int NFGuildModule::OnPropertyCommonEvent(const NFGUID & self, const std::string & strPropertyName, const NFData & oldVar, const NFData & newVar)
{
...
int NFGuildModule::OnRecordCommonEvent(const NFGUID & self, const RECORD_EVENT_DATA & xEventData, const NFData & oldVar, const NFData & newVar)
{
	return 0;
}

void NFGuildModule::SendMessageToGameServer(const NFGUID& guild, const int nMessageID, google::protobuf::Message& msg)
{
