...
void NFCreateRoleModule::AttachData(const NFGUID & self)
{
	...
	if (it != mxObjectDataCache.end())
	{
		...
		if (xObject)
		{
			...
			NFCommonRedisModule* pCommonRedisModule = static_cast<NFCommonRedisModule*>(m_pCommonRedisModule);

			if (xPropManager)
			{
				pCommonRedisModule->ConvertPBToPropertyManager(it->second.property(), xPropManager);
			}

			if (xRecordManager)
			{
				pCommonRedisModule->ConvertPBToRecordManager(it->second.record(), xRecordManager);
			}
...
void NFCreateRoleModule::SaveData(const NFGUID & self)
{
	...
	if (xObject)
	{
		...
		*(xDataPack.mutable_record()->mutable_player_id()) = NFINetModule::NFToPB(self);
		...
		if (xPropManager)
		{
			pCommonRedisModule->ConvertPropertyManagerToPB(xPropManager, xDataPack.mutable_property());
		}

		if (xRecordManager)
		{
			pCommonRedisModule->ConvertRecordManagerToPB(xRecordManager, xDataPack.mutable_record());
		}
