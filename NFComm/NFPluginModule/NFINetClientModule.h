...
public:
	...
	template<typename BaseType>
	int AddReceiveCallBack(NF_SERVER_TYPES eType, BaseType* pBase, void (BaseType::*handleRecieve)(const int, const int, const char*, const uint32_t))
	{
		NET_RECEIVE_FUNCTOR functor = std::bind(handleRecieve, pBase, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
		NET_RECEIVE_FUNCTOR_PTR functorPtr(new NET_RECEIVE_FUNCTOR(functor));
	...
	template<typename BaseType>
	bool AddEventCallBack(NF_SERVER_TYPES eType, BaseType* pBase, void (BaseType::*handler)(const int, const NF_NET_EVENT, NFINet*))
	{
		NET_EVENT_FUNCTOR functor = std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
		NET_EVENT_FUNCTOR_PTR functorPtr(new NET_EVENT_FUNCTOR(functor));
	...
	virtual void SendBySuit(const std::string& strHashKey, const int nMsgID, const std::string& strData) = 0;
	virtual void SendBySuit(NF_SERVER_TYPES eType, const std::string& strHashKey, const int nMsgID, const std::string& strData) = 0;

	virtual void SendBySuit(const std::string& strHashKey, const int nMsgID, const char* msg, const uint32_t nLen) = 0;
	virtual void SendBySuit(NF_SERVER_TYPES eType, const std::string& strHashKey, const int nMsgID, const char* msg, const uint32_t nLen) = 0;

	virtual void SendBySuit(const int& nHashKey, const int nMsgID, const std::string& strData) = 0;
	virtual void SendBySuit(NF_SERVER_TYPES eType, const int& nHashKey, const int nMsgID, const std::string& strData) = 0;

	virtual void SendBySuit(const int& nHashKey, const int nMsgID, const char* msg, const uint32_t nLen) = 0;
	virtual void SendBySuit(NF_SERVER_TYPES eType, const int& nHashKey, const int nMsgID, const char* msg, const uint32_t nLen) = 0;

	virtual void SendSuitByPB(const std::string& strHashKey, const uint16_t nMsgID, google::protobuf::Message& xData) = 0;
	virtual void SendSuitByPB(NF_SERVER_TYPES eType, const std::string& strHashKey, const uint16_t nMsgID, google::protobuf::Message& xData) = 0;

	virtual void SendSuitByPB(const int& nHashKey, const uint16_t nMsgID, google::protobuf::Message& xData) = 0;
	virtual void SendSuitByPB(NF_SERVER_TYPES eType, const int& nHashKey, const uint16_t nMsgID, google::protobuf::Message& xData) = 0;
	...
	virtual NFMapEx<int, ConnectData>& GetServerList(NF_SERVER_TYPES eType) = 0;
