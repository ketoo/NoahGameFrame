...
public:
    ...
    virtual bool SwitchScene(const NFGUID& self, const int nTargetSceneID, const int nTargetGroupID, const float fX, const float fY, const float fZ, const float fOrient, const NFDataList& arg);
    ...
    virtual int GetSceneOnLineList(const int nSceneID, NFDataList& var);
	...
	virtual int GetObjectByProperty(const int nSceneID, const std::string& strPropertyName, const NFDataList& valueArgArg, NFDataList& list);
