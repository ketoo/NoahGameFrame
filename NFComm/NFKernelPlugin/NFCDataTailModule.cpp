// -------------------------------------------------------------------------
//    @FileName			:    NFCDataTailModule.cpp
//    @Author           :    LvSheng.Huang
//    @Date             :    2013-09-30
//    @Module           :    NFCDataTailModule
//
// -------------------------------------------------------------------------
...
bool NFCDataTailModule::Shut()
{
    return true;
}

bool NFCDataTailModule::Execute()
{
    return true;
}

bool NFCDataTailModule::AfterInit()
{

    return true;
}

void NFCDataTailModule::StartTrail(const NFGUID& self)
{
	TrailObjectData(self);
}

void NFCDataTailModule::LogObjectData(const NFGUID& self)
{
...
int NFCDataTailModule::OnObjectPropertyEvent(const NFGUID& self, const std::string& strPropertyName, const NFData& oldVar, const NFData& newVar)
{
    std::ostringstream stream;
...
int NFCDataTailModule::OnObjectRecordEvent(const NFGUID& self, const RECORD_EVENT_DATA& xEventData, const NFData& oldVar, const NFData& newVar)
{
    std::ostringstream stream;
...
int NFCDataTailModule::TrailObjectData(const NFGUID& self)
{
    NF_SHARE_PTR<NFIObject> xObject = m_pKernelModule->GetObject(self);
    if (nullptr == xObject)
    {
    ...
    if (nullptr != xPropertyManager)
    {
        ...
        while (nullptr != xProperty)
        {
            m_pKernelModule->AddPropertyCallBack(self, xProperty->GetKey(), this, &NFCDataTailModule::OnObjectPropertyEvent);
    ...
    if (nullptr != xRecordManager)
    {
        ...
        while (nullptr != xRecord)
        {
            m_pKernelModule->AddRecordCallBack(self, xRecord->GetName(), this, &NFCDataTailModule::OnObjectRecordEvent);
...
void NFCDataTailModule::PrintStackTrace()
{
#if NF_PLATFORM != NF_PLATFORM_WIN
