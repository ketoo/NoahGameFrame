...
bool RecordFormString( NFIRecord* pRecord, const NFMsg::ObjectRecordBase& recordBase )
{
	...
	for ( int i = 0; i < recordBase.record_int_list_size(); i++ )
	{
		const NFMsg::RecordInt& recordInt = recordBase.record_int_list( i );
	...
	for ( int i = 0; i < recordBase.record_float_list_size(); i++ )
	{
		const NFMsg::RecordFloat& recordFloat = recordBase.record_float_list( i );
	...
	for ( int i = 0; i < recordBase.record_string_list_size(); i++ )
	{
		const NFMsg::RecordString& recordString = recordBase.record_string_list( i );
	...
	for ( int i = 0; i < recordBase.record_object_list_size(); i++ )
	{
		const NFMsg::RecordObject& recordObject = recordBase.record_object_list( i );
