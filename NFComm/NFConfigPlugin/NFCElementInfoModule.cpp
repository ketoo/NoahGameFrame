...
bool NFCElementInfoModule::Load(rapidxml::xml_node<>* attrNode, NFILogicClass* pLogicClass)
{
    ...
    if (pClassPropertyManager && pClassRecordManager)
    {
        ...
        while (pRecord)
        {
            pElementRecordManager->AddRecord(0, pRecord->GetName(), pRecord->GetInitData(), pRecord->GetKeyState(), pRecord->GetInitDesc(), pRecord->GetRows(), pRecord->GetPublic(), pRecord->GetPrivate(), pRecord->GetSave(), pRecord->GetIndex());
