		...
		html_ServerStatus_temp = "<div class='col-lg-2 col-sm-6'><div class='card'><div class='content'><div class='row'><div class='col-xs-5'><div class='icon-big {$Color} text-center'><i class='ti-light-bulb'></i></div></div><div class='col-xs-7'><div class='numbers'><p>{$ServerName}</p>{$ServerStatus}</div></div></div><div class='footer'><hr /><div class='stats'><i class='ti-reload'></i> {$UpdateTime}</div></div></div></div></div>";
		var html_ServerLine_temp;
		html_ServerLine_temp = "<div class='row'><div class='col-md-12'><div class='card'><div class='header'><h4 class='title'>{$serverName}</h4><p class='category'>Online Count</p></div><div class='content'><div id='{$serverNameId}' class='ct-chart'></div><div class='footer'><hr><div class='stats'><i class='ti-reload'></i> Updated 1 second ago</div></div></div></div></div></div>";
		var serverNowTime;
		var chartMaxMaster;
		var chartMaxWorld;
		var chartMaxGame;
		...
		{
			$.getJSON("http://" + $.cookie("formServerIP") + ":" + $.cookie("formServerPort") + "/id=10200,jsoncallback=?", function (data) {
	    ...
	    $("#serverButton").click(function(){  
	            $.cookie("formServerIP", $("#formServerIP").val() , { path: '/', expires: 10 });
	            $.cookie("formServerPort", $("#formServerPort").val() , { path: '/', expires: 10 });

				$.notify({
		            icon: 'ti-gift',
		            message: "Set IP["+$.cookie("formServerIP")+"] and Port["+$.cookie("formServerPort")+"] successfully."
		        },{
		            type: 'info',
		            timer: 10
		        });
